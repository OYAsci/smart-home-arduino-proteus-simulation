...
void setup() {
  ...
  servo1.attach(12); servo1.write(1);
...
void handleSecurity() {
  if (input == code) {
    servo2.write(130); delay(6000); servo2.write(180);
    input = "";
...
void handleAlarm(unsigned long now) {
  if (now - tStart < 10000) { digitalWrite(BUZZER, LOW); servo1.write(0); return; }

  if (temp > 50 || gas > 200) {
    if (now - lastBuzz >= 300) {
      buzzOn = !buzzOn;
      digitalWrite(BUZZER, buzzOn); servo1.write(50);servo2.write(130);
      lastBuzz = now;
    }
  } else {
    digitalWrite(BUZZER, LOW); servo1.write(0);servo2.write(180);
  }
}
